# -*- coding: utf-8 -*-
"""
Created on Sat Aug 17 16:22:54 2019
...
Links:
    https://martinos.org/mne/stable/generated/mne.stats.spatio_temporal_cluster_1samp_test.html
    https://martinos.org/mne/stable/auto_tutorials/stats-source-space/plot_stats_cluster_spatio
                _temporal.html#sphx-glr-auto-tutorials-stats-source-space-plot-stats-cluster-spatio-temporal-py
...
good_cluster_inds = np.where(p_values < 0.05)[0]  # p_accept = 0.05
...
#%% Make a code to only plot the topomap

    

#%% Visualise clusters w/ topomap + ave evoked
    
# configure variables for visualization
colors = {"BPint01l": "crimson"}
linestyles = {"BPint01l": '-'}
...
good_cluster_inds = good_cluster_inds[0]
...
#%% Save the array mask 
np.savetxt('mask_significant_grads.txt', maska, fmt='%d')
maska = np.loadtxt('mask_significant_grads.txt', dtype=bool)

# plot average test statistic and mark significant sensors
#plot_topomap(f_map, pos, mask=maska, cmap='Reds')
plot_topomap(f_map, pos, mask=maska, cmap='Reds')
