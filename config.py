...
Study Specific Parameters:
    ...
    runs : list of strings that specifies how your runs are named. 
            For example runs = ['run01', 'run02']. If you have only one run 
            it should be runs = [''].
    base_raw_fname : string that describes how your files are named. 
            For example: '{subject}_audvis_{run}_raw.fif'
            
    # BAD CHANNELS #
    bads : to be removed before maxfilter is applied (defined per run)
    
    # DEFINE ADDITIONAL CHANNELS #
    set_channel_types : set channel type of extra channels that were recorded 
            (e.g. EOG, ECG etc.) Example: set type for EEG062 as EOG.
    ...
    mf_reference_run : specify which run to use for HPI recalibration, all other 
            runs will have head position adjusted to this one.
    mf_head_origin : The origin of the head used for maxwell filtering.
    mf_st_duration : a float that specifies the buffer duration in seconds. 
            As an example, 10s acts like a 0.1 Hz highpass filter. 
            If None (default), no temporal spatial filtering is applied 
            during MaxFilter.
            
    # RESAMPLING #
    resample_sfreq : a float that specifies at which sampling frequency the data 
            should be resampled. 
    ...
    baseline : tuple that specifies how to baseline the epochs. 
            If None, then no baseline applied.
    event_id : python dictionary that maps events (trigger/marker values) to 
            conditions. E.g. event_id = {'Auditory/Left': 1, 'Auditory/Right': 2}
